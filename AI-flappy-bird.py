...
def score_display(screen, score, birds):
    score_surface = font.render(
        ("Score: " + str(score) + "  Nb de bird : " + str(birds)),
        True,
        (255,255,255)
    )
...
def draw(screen, birds, pipes):
    pygame.draw.rect(screen, (43, 45, 66), (0,0,screen_size, screen_size))
    for bird in birds:
        gfxdraw.filled_ellipse(screen, int(bird.pos["x"]), int(bird.pos["y"]), bird.width, bird.height, (255, 255, 255))
        gfxdraw.filled_ellipse(screen, int(bird.pos["x"]), int(bird.pos["y"]), bird.width-1, bird.height-1, bird.color)
    for pipe in pipes:
        pygame.draw.rect(screen, (141, 153, 174), (pipe.x ,0 , pipe.width, pipe.top_y))
        pygame.draw.rect(screen, (141, 153, 174), (pipe.x ,pipe.bottom_y , pipe.width, screen_size))
    ...
    while run:
        ...
        if not no_display:
            if frames%boost==0:
                ...
                score_display(screen, round(frames/10), len(birds))
